...
const shortcutWindows = ref(
  JSON.parse(localStorage.getItem(getCurrentWebviewWindow().label) || "{}")
);
...
listen("dellnk", (e: { payload: { label: string; lnk: string } }) => {
  ...
  if (e.payload.label == getCurrentWebviewWindow().label) {
    ...
    let i = shortcutWindows.value.shortcuts.findIndex(
      (item: { targetPath: any }) => {
...
watch(
  ...
  () => {
    localStorage.setItem(
      ...
      JSON.stringify(shortcutWindows.value)
    );
  },
  {
    deep: true,
...
const mouseleave = function (i: number) {
  ...
  if (el) {
    el.style.width =
      shortcutWindows.value.setting.w / shortcutWindows.value.setting.c -
      20 +
      "px";
    el.style.height =
      shortcutWindows.value.setting.w / shortcutWindows.value.setting.c -
