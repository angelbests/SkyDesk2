...
<template>
  <div class="container" id="container">
    <div
      id="contextmenu"
      style="position: absolute; z-index: 100; width: 100px; height: 100px; width: 120px; height: 90px; padding: 10px; display: flex; flex-direction: column; align-items: center; justify-content: space-evenly"
    ...
    <div class="search-bar">
      ...
      <input id="input" autocomplete="off" @input="search" v-model="inputvalue" type="text" @keyup.enter="searchenter" placeholder="请输入搜索内容" class="search-input" />
    </div>
    ...
    <div id="search-result" class="search-result" v-if="inputvalue">
      <div class="search-shortcut" :style="{ height: searchshortcut.length == 0 ? '0px' : '80px' }">
        <div tabindex="0" @keyup.enter="openshortcut(item)" v-for="item in searchshortcut" class="shortcut-container">
      ...
      <div class="search-system" id="search-system" :style="{ height: searchshortcut.length == 0 ? '440px' : '360px' }">
        <div
        ...
        <div v-else style="display: flex; justify-content: center; align-items: center; height: 100%">
          <div v-if="searchstatus" style="display: flex; flex-direction: column; justify-content: center; align-items: center">
    ...
    <div class="search-result" v-else>
      <v-tabs id="shortcuttab" density="compact" v-model="tab" center-active style="height: 36px; width: 100%; background: white" hide-slider show-arrows>
      ...
      <v-tabs-window v-model="tab" style="width: 100%; height: calc(100% - 86px)">
        <v-tabs-window-item v-for="item1 in shortcuts" style="width: 100%; height: 100%">
          <GridContainer v-model="item1.shortcut" :animation="150" :gridwidth="90" :gridheight="90" :group="{ name: 'shortcut', pull: 'clone' }">
            <template v-slot="{ item }">
              <div tabindex="0" @keyup.enter="openshortcut(item)" :style="{ height: '80px', backgroundSize: 'cover' }" class="shortcut-container">
                <div class="icon-div" @click="openshortcut(item)">
                  <img class="icon" :src="item.icoPath == '' ? '/icons/program.png' : convertFileSrc(item.icoPath)" />
                </div>
                <div style="font-size: 10px; width: 60px; height: 30px; filter: drop-shadow(0px 5px 5px gray); text-wrap: balance; text-align: center; text-overflow: clip; overflow: hidden">
      ...
      <div class="settingbottom">
        <div tabindex="0" v-for="item in settingbottom" class="settingbottom-item" @click="openPath(item.cmd)" @keyup.enter="openPath(item.cmd)">
        ...
        <div tabindex="0" class="settingbottom-item" @click="screen" @keyup.enter="screen">
          ...
          <v-tooltip activator="parent" location="top">截图</v-tooltip>
        </div>
        <div tabindex="0" class="settingbottom-item" @click="createnote" @keyup.enter="createnote">
          ...
          <v-tooltip activator="parent" location="top">便签</v-tooltip>
        </div>
      </div>
    </div>
  </div>
