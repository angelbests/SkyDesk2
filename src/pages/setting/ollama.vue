    ...
    <v-dialog v-model="loadsetup" width="auto" data-tauri-drag-region>
      <v-card prepend-icon="mdi-update" title="安装ollama">
        ...
        <template v-slot:actions>
          <v-btn style="margin-right: 20px" @click="checkollama()">安装完成</v-btn>
          <v-btn style="margin-right: 20px" @click="closesetup">暂不使用</v-btn>
        </template>
      </v-card>
    </v-dialog>
    <v-card
      ...
      <v-tabs-window v-model="model" style="width: 100%; height: 100%">
        ...
        <div
          ...
          <template v-for="item in messages">
            <div v-if="item.role == 'assistant'" style="width: 100%; display: flex; justify-content: flex-start; margin-bottom: 20px; transition: all 0.3s linear">
              <div style="width: calc(100% - 50px); display: flex; justify-content: flex-start">
                <v-avatar icon="mdi-robot-outline" style="margin-right: 10px" :style="{ color: systemstore.fontcolor }"></v-avatar>
                <div
                  class="markdown-body"
                  :style="{
            ...
            <div v-else style="width: 100%; display: flex; justify-content: flex-end; margin-bottom: 20px; transition: all 0.3s linear">
        ...
        <div style="width: 100%; height: 60px; display: flex; justify-content: center; align-items: flex-end; box-sizing: border-box; padding: 10px 10px">
          <v-text-field width="50px" v-model="message" hide-details="auto" density="compact" variant="solo" clearable v-on:keyup.enter="sendmessage">
