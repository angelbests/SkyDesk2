...
const watchScanProgram = async function () {
  let arr = await get_uwp()
  for (let i = 0; i < arr.length; i++) {
    console.log(arr[i].lnkPath)
  }
...
const submitshortcut = async function () {
  if (shortcut.value.lnkPath && shortcut.value.icoPath && shortcut.value.name) {
    ;(shortcuts.value[tab.value].shortcut[index.value].type = 'openPath'), (shortcuts.value[tab.value].shortcut[index.value].lnkPath = shortcut.value.lnkPath)
...
const submitshortcut2 = async function () {
  if (shortcut.value.lnkPath && shortcut.value.name && shortcut.value.icoPath) {
...
const adddown = function () {
  let map = new Map()
  wheels.value = wheels.value.filter((v) => !map.has(v.name) && map.set(v.name, v))
}

// 拖拽到右边
const addright = function () {
  let map = new Map()
  shortcuts.value[tab.value].shortcut = shortcuts.value[tab.value].shortcut.filter((v) => !map.has(v.name) && map.set(v.name, v))
}

// 拖拽回左边
const addleft = function () {
  let map = new Map()
  shortcutsTemp.value = shortcutsTemp.value.filter((v) => !map.has(v.name) && map.set(v.name, v))
}
...
<template>
  <div id="shortcut" style="width: 100%; height: 100%; position: relative">
    ...
    <v-dialog max-width="500" v-model="dialog">
      <v-card>
        <v-list>
          ...
          <v-list-item>
            <v-text-field v-model="shortcut.lnkPath" density="compact" hide-details="auto" :readonly="true" @click="getlnk" :label="selecttype == 'file' ? '选择文件' : '选择文件夹'"></v-text-field>
          </v-list-item>
          <v-list-item>
            <v-text-field v-model="shortcut.icoPath" density="compact" hide-details="auto" :readonly="true" @click="getico" label="图标路径"></v-text-field>
          </v-list-item>
          <v-list-item>
      ...
      <v-card style="width: 100%; height: 100%">
        <v-card-text>
          <div style="display: flex; flex-direction: row; width: 100%; height: 100%">
            ...
            <div
              id="dragfile"
              style="width: 150px; height: 90%; margin-top: 3%; background-image: linear-gradient(to top, #cfd9df 0%, #e2ebf0 100%); display: flex; justify-content: center; align-items: center; color: gray">
              拖入
            </div>
          </div>
        </v-card-text>
    ...
    <div @dragover="dragover($event)" @drop="drop($event)" style="width: 100%; height: calc(100% - 180px); display: flex; overflow: hidden">
      <!-- 左 -->
        ...
        <GridContainer
          ...
          <template v-slot="{ item }">
            <div
              ...
              <div style="font-size: 10px; width: 60px; height: 30px; text-wrap: balance; text-align: center; text-overflow: clip; overflow: hidden; line-height: 15px; filter: drop-shadow(0px 5px 5px gray)">
                {{ item.name }}
              </div>
            </div>
          </template>
      ...
      <div
        ...
        <v-tabs id="shortcuttab" density="compact" v-model="tab" center-active style="height: 36px; width: 100%; background: rgba(223, 223, 223, 0.4)" hide-slider show-arrows>
        ...
        <div style="width: 100%; height: calc(100% - 36px)">
          <v-tabs-window v-model="tab" style="width: 100%; height: 100%; padding: 10px">
            <v-tabs-window-item v-for="item1 in shortcuts" style="width: 100%; height: 100%; min-height: 100%">
              <GridContainer v-model="item1.shortcut" :animation="150" :gridwidth="90" :gridheight="setting ? 120 : 90" :group="{ name: 'shortcut', pull: 'clone' }" :setData="setdata" :clone="clone" :onAdd="addright">
                <template v-slot="{ item, index }">
                  <div
                    ...
                    <div style="font-size: 10px; width: 60px; height: 30px; filter: drop-shadow(0px 5px 5px gray); text-wrap: balance; text-align: center; text-overflow: clip; overflow: hidden">
                      {{ item.name }}
                    </div>
                    <div v-show="setting" style="width: 80px; height: 20px; display: flex; justify-content: space-around; align-items: flex-end">
                      <v-btn @click="delSshorcut(item)" style="font-size: 12px; box-shadow: none; background: none; color: gray" size="mini">
                        <template v-slot:prepend>
                          <v-icon>mdi-delete-outline</v-icon>
                        </template>
                        删
                      </v-btn>
                      <v-btn @click="editshortcut(index)" style="font-size: 10px; box-shadow: none; background: none; color: gray" size="mini">
                        <template v-slot:prepend>
    ...
    <!-- 下 -->
    <VueDraggable v-model="shortcutstore.wheels" :group="{ name: 'shortcut' }" :animation="150" class="VueDraggable-wheel" :onAdd="adddown" :setData="setdata">
      <div
        :style="{
          background: systemstore.shortcutbackground,
          backgroundSize: 'cover',
        ...
        <div style="font-size: 10px; width: 70px; height: 30px; filter: drop-shadow(0px 5px 5px gray); text-wrap: balance; text-align: center; text-overflow: clip; overflow: hidden; line-height: 15px">
          {{ item.name }}
        </div>
      </div>
    </VueDraggable>
