...
const repeat = [{ title: '不重复', value: 0 }, { title: '每天', value: 1 }, { title: '每周', value: 2 }, { title: '每月', value: 3 }, { title: '每年', value: 4 }]
...
const weekday = [{ title: '周一', value: 1 }, { title: '周二', value: 2 }, { title: '周三', value: 3 }, { title: '周四', value: 4 }, { title: '周五', value: 5 }, { title: '周六', value: 6 }, { title: '周天', value: 7 }]
    ...
    let arr = schedules.value.filter(item => {
        ...
        if (item.repeat == 0) {
            ...
            if((daystart<day.cDay && monthstart<(day.cMonth-1)) && (dayend>day.cDay && monthend>(day.cMonth-1))){
                item.first = false
                return true
            }else if((daystart==day.cDay && monthstart==(day.cMonth-1)) && (dayend==day.cDay && monthend==(day.cMonth-1))){
                item.first = true
                return true
            }else{
                item.first = false
...
<template>
    <div class="window">
            ...
            <v-card>
                ...
                <template v-slot:text>
                    <v-list>
                        ...
                        <v-list-item>
                            <v-text-field v-model="schedule.title" label="标题" density="compact"
                                hide-details="auto"></v-text-field>
                        </v-list-item>
                        <v-list-item>
                            <v-text-field v-model="schedule.address" label="地点" density="compact"
                                hide-details="auto"></v-text-field>
                        </v-list-item>
                        <v-list-item v-show="schedule.repeat == 2">
                            <v-select v-model="schedule.week" density="compact" hide-details="auto" :items="weekday" label="周" multiple>
                        ...
                        <v-list-item v-show="schedule.repeat == 2 || schedule.repeat == 1">
                            <v-select v-model="schedule.start.time" style="width: 100%;" density="compact" hide-details="auto" label="时间" :items="gettimearr()"></v-select>
                        </v-list-item>
                        <v-list-item v-show="(schedule.repeat != 2 && schedule.repeat != 1)">
                            <div style="width: 100%;display: flex;">
                                <v-date-input v-model="schedule.start.date" @update:model-value="startupdate"
                                    cancel-text="取消" ok-text="确定" style="width: 50%;" density="compact"
                                    hide-details="auto" label="开始日期" prepend-icon=""></v-date-input>
                                <v-select v-model="schedule.start.time" style="width: 50%;" density="compact"
                                    hide-details="auto" label="时间" :items="gettimearr()"></v-select>
                            </div>
                        </v-list-item>
                        <v-list-item v-show="(schedule.repeat != 2 && schedule.repeat != 1)">
                            <div style="width: 100%;display: flex;">
                                <v-date-input :min="minend" :max="maxend" v-model="schedule.end.date" @update:model-value="endupdate" cancel-text="取消"
                                    ok-text="确定" style="width: 50%;" density="compact" hide-details="auto" label="结束日期"
                                    prepend-icon=""></v-date-input>
                                <v-select v-model="schedule.end.time" style="width: 50%;" density="compact"
                                    hide-details="auto" label="时间" :items="gettimearr()"></v-select>
                            </div>
                        </v-list-item>
                        <v-list-item>
                        ...
                        <v-list-item>
                            <v-textarea v-model="schedule.text" density="compact" hide-details="auto"
                                label="描述"></v-textarea>
                        </v-list-item>
                    </v-list>
                </template>
        ...
        <v-card @wheel.self="wheelmonth"
            ...
            <div
                style="display: flex;flex-direction: row;justify-content: center;align-items: center;font-size: 20px;height:100%;margin-right: 20px;width: 150px;">
        ...
        <div @wheel="wheelmonth" style="width: 100%;height: calc(100% - 64px);display: flex;overflow: hidden;background: white;">
            <v-month v-model:date="date">
                <template v-slot:default="{day}">
                    <div
                        :style="{ width: '100%', height: '20px', textAlign: 'center', background: (new Date().getFullYear() == day.cYear) && (new Date().getMonth() + 1 == day.cMonth) && (new Date().getDate() == day.cDay) ? 'rgba(133,133,133,1)' : 'rgba(233,233,233,1)' }">
                        {{ day.cDay }}
                        {{ day.IMonthCn }}
                        {{ day.IDayCn }}
                    ...
                    <div v-for="item in filterschedule(day)"
