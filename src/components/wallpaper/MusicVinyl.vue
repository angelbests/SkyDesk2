...
smtc.listen_appstatus(async (e) => {
  if (!e.payload.cloudmusic && wallpaperstore.wallpaperConfig[index.value].config.musicapp == 'cloudmusic.exe') {
    media.value.thumb = ''
  } else if (!e.payload.qqmusic && wallpaperstore.wallpaperConfig[index.value].config.musicapp == 'QQMusic.exe') {
    media.value.thumb = ''
  } else if (!e.payload.spotify && wallpaperstore.wallpaperConfig[index.value].config.musicapp == 'Spotify.exe') {
    media.value.thumb = ''
  }
})
...
<template>
  ...
  <div class="music_vinyl" v-show="media.thumb" :style="{ background: `url(${media.thumb})`, backgroundSize: 'cover' }">
    <svg id="tonearm" style="position: absolute; z-index: 300" width="400" height="300" viewBox="0 0 400 300">
      <g id="tonearmg" transform="rotate(0,280,40)" style="filter: drop-shadow(0px 0px 2px black)">
        <path d=" M 282 10 C 270 230, 270 200, 245 235" stroke="#304352" fill="none" stroke-width="4" stroke-linecap="round"></path>
        <g style="filter: drop-shadow(0px 0px 1px black)">
          <rect x="270" y="120" width="14" height="20" fill="gray" transform="rotate(50,144,144)" rx="2" ry="2" />
        </g>
        <circle cx="280" cy="40" r="10" fill="gray" />
        <circle cx="280" cy="40" r="4" fill="#304352" />
        <animateTransform id="tonearmanimate" attributeName="transform" attributeType="XML" type="rotate" values="0,280,40;2,280,40;0,280,40" dur="6s" repeatCount="indefinite" fill="freeze" begin="indefinite" />
      </g>
    </svg>
    <div id="vinyl1" class="vinyl">
      <!-- 封面 -->
      <img id="music_img" :style="{ animationPlayState: playstatus == 4 ? 'running' : 'paused' }" :src="media.thumb" class="music_pic_img" />
      <!-- 时间线进度 -->
      <v-progress-circular class="music-progress" bg-color="#00f2fe" color="#304352" :model-value="(timeline.position / timeline.end) * 100" :width="10" :size="190"></v-progress-circular>
      <div class="circle_border" :style="{ animationPlayState: playstatus == 4 ? 'running' : 'paused' }"></div>
    </div>
    <div id="vinyl2" class="vinyl">
      <!-- 封面 -->
      <img id="music_img" :style="{ animationPlayState: playstatus == 4 ? 'running' : 'paused' }" :src="new_thumb" class="music_pic_img" />
      <!-- 环形柱状图 -->
      <canvas v-show="playstatus == 4 ? true : false" id="music_canvas" class="music_canvas" width="270" height="270"></canvas>
      <!-- 时间线进度 -->
      <v-progress-circular class="music-progress" bg-color="#00f2fe" color="#304352" tag="div" :model-value="(timeline.position / timeline.end) * 100" :width="10" :size="190"></v-progress-circular>
      <div class="circle_border" :style="{ animationPlayState: playstatus == 4 ? 'running' : 'paused' }"></div>
    </div>
  </div>
</template>
