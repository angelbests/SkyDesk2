...
import { appDataDir, basename, extname, resolve, resourceDir } from "@tauri-apps/api/path"
import { copyFile, exists, readDir, readFile, readTextFile, writeFile } from "@tauri-apps/plugin-fs"
import { uuid } from "."
  ...
  for (let i = 0; i < lnks.length; i++) {
    if (pattern.test(lnks[i].iconLocationPeFile)) {
      ...
      if (pexec) {
        ...
        if (systemPath) {
          lnks[i].iconLocationPeFile = (lnks[i].iconLocationPeFile as string).replace(systemPara, systemPath)
        }
      }
    }
    if (pattern.test(lnks[i].targetPath)) {
      ...
      if (pexec) {
        ...
        let systemPath = await getSystemPath(systemPara)
        if (systemPath) {
          lnks[i].targetPath = (lnks[i].targetPath as string).replace(systemPara, systemPath)
        }
      }
    }
  }
  ...
  for (let i = 0; i < lnks.length; i++) {
    ...
    if (ext == "lnk") {
      ...
      if (lnks[i].iconLocationPeFile == "") {
        ...
        info("有图标文件")
        lnks[i].icoPath = await iconLocationPeFilegetico(lnks[i].iconLocationPeFile, lnks[i].iconLocation)
      }
    } else if (ext == "url") {
      // url快捷方式
...
const iconLocationPeFilegetico = async function (pe: string, iconLocation: string) {
  let path = await resolve(await appDataDir(), "ico")
  let defaultico = (await resourceDir()) + "/resources/program.png"
  let ext = ""
  try {
  ...
  if (ext == "ico" || ext == "png") {
    ...
    if (icodir) {
      if (Number(iconLocation) >= 0) {
        ...
        let path = icodir + name + "_" + Math.abs(Number(iconLocation)) + ".ico"
...
const targetPathgetico = async function (pe: string) {
  ...
  } else if (ext == "mp4" || ext == "flv" || ext == "mkv" || ext == "mov" || ext == "wmv" || ext == "webm") {
    info(`视频格式`)
